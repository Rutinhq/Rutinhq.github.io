...
- **Blog:** `/blog` (canonical `https://www.rutinhq.com/blog`) + Article01 are **indexable** and listed in `sitemap.xml` (6 www URLs). Build prerenders crawl shells (`dist/blog.html` + `dist/prerender/blog-icp-gated-cold-outbound-without-rented-sdr.html`) and `_redirects` 200-rewrites the pretty URLs so `curl` sees unique title/canonical/OG without JS. Do **not** emit `dist/blog/index.html` (CF would 308 `/blog`). Soft-park Store/NEXUS on the blog footer only. No 3-SKU strip on `/blog`.
